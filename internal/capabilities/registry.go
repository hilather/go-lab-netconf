...
// LookupResource returns the capability bound to a labnetconf:// URI template.
...
func ValidateCatalog() error {
	...
	for _, c := range all {
		...
		for _, t := range c.MCP.Tools {
			if strings.HasPrefix(t, "labnetconf_") {
				return fmt.Errorf("%s: tool %s must use netconf_ prefix, not labnetconf_", c.ID, t)
			}
			if !strings.HasPrefix(t, "netconf_") {
				return fmt.Errorf("%s: tool %s must use netconf_ prefix", c.ID, t)
			}
		}
		if c.MCP != nil {
			for _, r := range c.MCP.Resources {
				if !strings.HasPrefix(r, "labnetconf://") {
					return fmt.Errorf("%s: resource %s must use labnetconf://", c.ID, r)
				}
			}
		}
	}
