...
const realmBearer = `Bearer realm="labnetconf"`
...
func FromSpec(spec model.AuthSpec) (*Verifier, error) {
	...
	if mode != model.MgmtAuthBearer {
		return nil, domainerr.ValidationFailed("unknown auth mode",
			domainerr.FieldViolation{Path: "spec.auth.mode", Code: "invalid_value", Message: "mode must be bearer"})
	}
...
func (v *Verifier) Authenticate(in Request) (Principal, error) {
	if v == nil {
		return Principal{}, domainerr.Unauthorized("authentication required")
	}
	v.mu.RLock()
	defer v.mu.RUnlock()
