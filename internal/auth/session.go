...
const (
	...
	CookieName = "labnetconf_session"
	// CSRFHeader is required on cookie-authenticated mutations.
	CSRFHeader = "X-LabNETCONF-CSRF"
	cookiePath = "/"
)
...
func (s *Store) Create(p Principal) (cookieValue, csrf string, sess Session, err error) {
	if s == nil {
		return "", "", Session{}, domainerr.ValidationFailed("session store unavailable")
	}
...
func randomHex(n int) (string, error) {
	...
	if _, err := rand.Read(b); err != nil {
		return "", domainerr.ValidationFailed("session material unavailable")
	}
