...
func rpcError(err error) error {
	...
	if mapped.Data != nil {
		...
		if mErr != nil {
			raw = []byte(`{"code":"validation_failed","message":"internal error","retryable":false}`)
		}
		data = raw
	}
...
func toolErrorResult(err error) *sdk.CallToolResult {
	...
	if sErr != nil {
		structured = map[string]any{"code": "validation_failed", "message": "internal error", "retryable": false}
	}
