...
func TestParityEveryRequiredRowHasMCPTool(t *testing.T) {
	...
	for _, c := range capabilities.All() {
		...
		for _, name := range c.MCP.Tools {
			if !strings.HasPrefix(name, "netconf_") {
				t.Errorf("%s tool %s must use netconf_ prefix", c.ID, name)
			}
			if strings.HasPrefix(name, "labnetconf_") {
				t.Errorf("%s tool %s uses rejected labnetconf_ prefix", c.ID, name)
			}
			if !live[name] {
	...
	for name := range live {
		found := false
		for _, want := range capabilities.Tools() {
			if want == name {
				found = true
				break
			}
...
func TestParityEveryRequiredResourceIsLive(t *testing.T) {
	s, _ := newTestServer(t)
	ts := startHTTP(t, s)
	cs := connectClient(t, ts)
	live := map[string]bool{}
	...
	for _, uri := range capabilities.Resources() {
		if !live[uri] {
			t.Errorf("missing twin resource %s", uri)
		}
	}
	for uri := range live {
		found := false
		for _, want := range capabilities.Resources() {
			if want == uri {
				found = true
				break
			}
...
func TestAllowLegacyClientsDefaultFalse(t *testing.T) {
	svc := bootTestApp(t)
	s, err := New(Config{
		Service: svc,
	...
	if s.cfg.AllowLegacyClients {
		t.Fatal("allowLegacyClients default must be false")
	}
}
