...
func testFS() fstest.MapFS {
	return fstest.MapFS{
		"index.html":                   {Data: []byte("<!doctype html><title>LabNETCONF</title>")},
...
func TestSPAFallbackAndCache(t *testing.T) {
	...
	if !strings.Contains(index.Body.String(), "LabNETCONF") {
	...
	h.ServeHTTP(fallback, httptest.NewRequest(http.MethodGet, "/datastores", nil))
...
func TestFilesHasIndex(t *testing.T) {
	...
	if !strings.Contains(string(raw), "LabNETCONF") {
...
func TestStubFiles(t *testing.T) {
	...
	if !strings.Contains(rec.Body.String(), "LabNETCONF") {
