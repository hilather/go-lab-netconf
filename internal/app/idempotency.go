...
func (c *idempCache) lookup(key, fp string) (*idempEntry, error) {
	...
	if e.fp != fp {
		return nil, domainerr.ValidationFailed("idempotency key reused with a different request",
			domainerr.FieldViolation{Path: "idempotencyKey", Code: "invalid_value", Message: "idempotency key reused with a different request"})
	}
	c.moveFrontLocked(e)
	return e, nil
}
...
func fingerprintOps(ops []ApplyOp) (string, error) {
	b, err := json.Marshal(ops)
	if err != nil {
		return "", domainerr.ValidationFailed("idempotency fingerprint: " + err.Error())
	}
	sum := sha256.Sum256(b)
...
func clonePlan(p Plan) Plan {
	...
	if p.Operations != nil {
		out.Operations = append([]ApplyOp(nil), p.Operations...)
	}
	return out
}
